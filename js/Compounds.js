AFRAME.registerComponent("atoms", {
  ...
  createAtoms: async function (element) {
    ...
    for(var num = 1; num <= numOfElectron; num ++){
      ...
      orbit.setAttribute("geometry", {primitive: "torus", arc: 360, radius: 0.28, radiusTubular: 0.001})
      ...
      //create electron revolving animation
      var electronMove = document.createElement("a-entity")
      electronMove.setAttribute("id", `electron-group-${elementName}`)
      electronMove.setAttribute("rotation", {x: 0, y: 0, z: electronAngle})
      electronAngle += 65
      electronMove.setAttribute("animation", {property: "rotation", to: `0 0 -360`, loop: "true",  dur: 3500, easing: "linear"})
      orbit.appendChild(electronMove)
      ...
      electronMove.appendChild(electron)
  ...
  createModel: function (model) {
    ...
    height: model.height
    modelEl.setAttribute("position", model.position); modelEl.setAttribute("rotation", model.rotation); modelEl.setAttribute("material", {
    });
    color: model.color
    marker.appendChild(modelEl);
    } else {
    var modelEl = document.createElement("a-entity"); modelEl.setAttribute("id", `${modelName}`);
